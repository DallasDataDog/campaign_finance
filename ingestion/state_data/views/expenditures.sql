CREATE OR REPLACE VIEW CAMPAIGN_FINANCE.EXPENDITURES_DALLAS_COUNTY_2023 AS 
SELECT 
    ...
    UPPER(TRIM(EXPENDITURES.EXPEND_DESCR)) AS EXPEND_DESCR,
    ...
    UPPER(COALESCE(PAYEE_NAME_ORGANIZATION, CONCAT(PAYEE_NAME_FIRST,' ',PAYEE_NAME_LAST))) AS PAYEE_FULL_NAME,
    UPPER(TRIM(EXPENDITURES.PAYEE_NAME_ORGANIZATION)) AS PAYEE_NAME_ORGANIZATION,
    UPPER(TRIM(EXPENDITURES.PAYEE_NAME_FIRST)) AS PAYEE_NAME_FIRST,
    UPPER(TRIM(EXPENDITURES.PAYEE_NAME_LAST)) AS PAYEE_NAME_LAST,
    UPPER(TRIM(EXPENDITURES.PAYEE_NAME_SUFFIX_CD)) AS PAYEE_NAME_SUFFIX_CD,
    EXPENDITURES.PAYEE_NAME_PREFIX_CD,
    UPPER(TRIM(EXPENDITURES.PAYEE_NAME_SHORT)) AS PAYEE_NAME_SHORT,
    UPPER(TRIM(EXPENDITURES.PAYEE_STREET_ADDR1)) AS PAYEE_STREET_ADDR1,
    UPPER(TRIM(EXPENDITURES.PAYEE_STREET_ADDR2)) AS PAYEE_STREET_ADDR2,
    UPPER(TRIM(EXPENDITURES.PAYEE_STREET_CITY)) AS PAYEE_STREET_CITY,
    UPPER(TRIM(EXPENDITURES.PAYEE_STREET_STATE_CD)) AS PAYEE_STREET_STATE_CD,
    UPPER(TRIM(EXPENDITURES.PAYEE_STREET_COUNTY_CD)) AS PAYEE_STREET_COUNTY_CD,
...
FROM (SELECT * FROM EXPENDITURES WHERE YEAR(EXPEND_DT) >= '2023') EXPENDITURES
LEFT JOIN DALLAS_COUNTY_CANDIDATES CANDIDATES
    ON EXPENDITURES.FILER_IDENT = CANDIDATES.TEC_FILER_IDENT
JOIN (
    SELECT FILER_IDENT, FILER_NAME_FIRST, FILER_NAME_SHORT, FILER_NAME_LAST, FILER_TYPE_CD,CTA_SEEK_OFFICE_CD,CTA_SEEK_OFFICE_DISTRICT
    FROM FILER_INFO
    WHERE
