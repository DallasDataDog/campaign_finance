...
conn = sf.sf_auth('DCED_ELECTIONS')
...
sf.sf_upload_file_to_stage(conn, current_dir / 'seed/','dallas_city_council_elections.csv','RAW_FILES')
sf.sf_upload_file_to_stage(conn, current_dir / 'seed/','dallas_county_primary_elections.csv','RAW_FILES')

#generate script for create table and copy into
with open(current_dir / 'tables/dallas_election_results.sql') as script:
